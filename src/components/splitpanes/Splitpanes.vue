...
export default {
  ...
  methods: {
    ...
    onMouseMove (e) {
      if (this.touch.mouseDown) {
        ...
        this.$emit('resize', this.panes.map(pane => ({
          min: pane.min,
          max: pane.max,
          width: pane.width
        })))
    ...
    onMouseUp () {
      if (this.touch.dragging) {
        this.$emit('resized', this.panes.map(pane => ({
          min: pane.min,
          max: pane.max,
          width: pane.width
        })))
    ...
    onSplitterClick (e, splitterIndex) {
      if ('ontouchstart' in window) {
        ...
        if (splitter !== splitterIndex) {
          ...
          this.onSplitterDblClick(e, splitterIndex)
        }
      }
      if (!this.touch.dragging) {
    ...
    getCurrentDragPercentage (drag) {
      ...
      const scrollTop = this.isVertical ? (window.pageYOffset || doc.scrollTop) - (doc.clientTop || 0) : null
      const offset = this[`getContainerOffset${this.isVertical ? 'Top' : 'Left'}`]()
      drag = drag[this.isVertical ? 'y' : 'x'] - offset + scrollTop
      // In the code bellow 'size' refers to 'width' for vertical and 'height' for horizontal layout.
      const containerSize = this.container.vnode[this.isVertical ? 'clientHeight' : 'clientWidth']
    ...
    calculatePanesSize (drag) {
      ...
      const minDrag = 0 + (this.isExpansive ? 0 : sums.prevPanesSize)
      const maxDrag = 100 - (this.isExpansive ? 0 : sums.nextPanesSize)
      ...
      // When isExpansive = true, find the closest expanded pane on each side of the splitter.
      if (this.isExpansive) {
        let vars = this.pushOtherPanes(sums, dragPercentage)
    ...
    pushOtherPanes (sums, dragPercentage) {
      const splitterIndex = this.touch.activeSplitter
      let panesToResize = [splitterIndex, splitterIndex + 1]
      // Pushing Down.
    ...
    findPrevExpandedPane (splitterIndex) {
      ...
      let arr = [...this.panes]
      arr.reverse().some(p => {
        if (p.index < splitterIndex && p.width > p.min) {
          pane = p
        }
    ...
    findNextExpandedPane (splitterIndex) {
      let pane = {}
      this.panes.some(p => {
        if (p.index > splitterIndex + 1 && p.width > p.min) {
          pane = p
        }
    ...
    if (this.panes.length) {
      this.$slots.default.forEach(vnode => {
        ...
        if (className === 'splitpanes__pane' && (id = id.replace('pane_', '')) && this.panes[id] &&
          ...
          const {
            elm: { attributes: { 'panes-size': { value: paneSizeInDOM } = {} } = {} } = {}
          } = vnode
          if (paneSizeInDOM !== undefined) {
            this.panes[id].savedWidth = parseFloat(paneSizeInDOM)
          } else {
            this.panes[id].savedWidth = parseFloat(width || height)
          }
        }
      })
    }
  ...
  render (createEl) {
    ...
    if (!this.$slots.default) {
      splitPanesChildren.push(createEl('div', 'Splitpanes needs some content here.'))
    } else {
      ...
      if (this.slotsCount !== this.$slots.default.length || slotsHaveChanged) {
        ...
        this.vnodes.forEach((vnode, i) => {
          ...
          const {
            'panes-min': min = 0,
            'panes-max': max = 100,
            'panes-size': Default = this.defaultWidth
          } = attrs
      ...
      this.vnodes.forEach((vnode, i) => {
        ...
        if (i) {
          let splitterAttributes = {
            ...
            ref: `splitter-${i - 1}`,
            style: this.splitpanesSplitterStyle,
            on: {
        ...
        let paneAttributes = {
          ...
          style: {
            ...(this.isVertical ? { height: `${this.panes[i].width}%` } : null),
            ...(!this.isVertical ? { width: `${this.panes[i].width}%` } : null)
          },
          on: {
            click: () => this.$emit('pane-click', this.panes[i])
          }
        }
    ...
    let wrapperAttributes = {
      class: [
        'splitpanes',
        `splitpanes--${this.isVertical ? 'vertical' : 'horizontal'}`,
